...
Thanks to organisations such as [OSGeo](https://www.osgeo.org/) and the wider open source movement, geospatial analysis is no longer the preserve of those with expensive hardware and software: anyone can now download and run high performance spatial libraries.
...
This can have the unintended consequence of discouraging reproducibility, as illustrated in Table \@ref(tab:gdsl).
...
The reproducible and 'computational' workflows enabled by R's CLI can also help unleash its statistical capabilities on geographic data (see section \@ref(why-geocomputation-with-r) and the note below).
...
Instead, we use the term to refer to 'working with geographic data in a computational way, focussing on code, reproducibility and modularity'.
...
Table: (\#tab:revdep)The top 5 most downloaded packages that depend on sf, in terms of average number of downloads per day over the previous month. As of 2018-09-28 there are 102 packages which import sf.
